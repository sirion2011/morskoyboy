...
async def state_handler(message: types.Message):
    ...
    if not existing_user:
        # Проверяем есть ли параметр после /start (реферальная ссылка)
        args = message.text.split()
        if len(args) > 1:
            # БАГ ИСПРАВЛЕН: используем args[1] вместо message.text[7:]
            try:
            ...
            if referral_id:
                try:
                    ...
                    logger.warning(f"Не удалось отправить реферальное уведомление: {e}")
        else:
            await User.create(
                tg_id=message.from_user.id,
                name=message.from_user.first_name,
