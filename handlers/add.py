...
from aiogram.types import reply_keyboard_remove
...
async def state_handler31(message: types.Message, state: FSMContext):
    data = await state.get_data()
    try:
        ad = await bot.copy_message(chat_id=father_id, from_chat_id=father_id,
...
async def state_handler(message: types.Message, state: FSMContext):
    ...
    text = message.text
    if text == f'Нет 3':
        await bot.send_message(chat_id=user_id, text=f'Сообщение удалено', reply_markup=main_menu.MainMenu)
    else:
        ...
        for user in users:
            if dictic:
                try:
                    ad = await bot.copy_message(chat_id=user.tg_id, from_chat_id=father_id, message_id=message_id,
                                           reply_markup=make_keyboard(dictic))
                    if timedelta in [24, 48]:
                        scheduler = AsyncIOScheduler()
                        scheduler.add_job(ad_deleter, 'date',
                                          run_date=datetime.now() + timedelta(hours=delta),
                                          args=[message.from_user.id, ad.message_id, scheduler])
                        scheduler.start()
                except Exception as e:
                    pass
                ...
                try:
                    ...
                    scheduler = AsyncIOScheduler()
                    scheduler.add_job(ad_deleter, 'date',
                                      run_date=datetime.now() + timedelta(seconds=delta),
                                      args=[message.from_user.id, ad.message_id, scheduler])
                    scheduler.start()
                except Exception as e:
                    pass
